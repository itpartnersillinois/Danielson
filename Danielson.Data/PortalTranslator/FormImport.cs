...
namespace Danielson.Data.PortalTranslator {
    ...
    public class FormImport {
        ...
        public async Task<int> Save(Form form, string email) {
            using var connection = new SqlConnection(_connectionString);
            return await connection.ExecuteAsync("[danielson].[usp_update_evaluation] @evaluation_id, @evaluation_cte_stu_id, @modified_by, @evaluator_netid, @isMidterm, @domain1_a, @domain1_b, @domain1_c, @domain1_d, @domain1_e, @domain1_f, @domain1_g, @domain1_strengths, @domain1_next_steps, @domain2_a, @domain2_b, @domain2_c, @domain2_d, @domain2_e, @domain2_strengths, @domain2_next_steps, @domain3_a, @domain3_b, @domain3_c, @domain3_d, @domain3_e, @domain3_f, @domain3_g, @domain3_strengths, @domain3_next_steps, @domain4_a, @domain4_b, @domain4_c, @domain4_d, @domain4_e, @domain4_f, @domain4_g, @domain4_strengths, @domain4_next_steps, @summary, @signature, @summary_code, @form_template_code",
                    new {
                        evaluation_id = form.AssignmentId,
                        evaluation_cte_stu_id = form.StudentEvaluationId,
                        modified_by = email,
                        ...
                        domain3_f = GetFormDomainValue(form, DomainEnum.Three, 6),
                        domain3_g = 0,
